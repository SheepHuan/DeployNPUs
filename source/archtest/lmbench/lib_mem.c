...
void base_initialize(iter_t iterations, void *cookie)
{
...
void mem_initialize(iter_t iterations, void *cookie)
{
	...
	for (i = 0; i < npages; ++i)
	{
		for (j = 0; j < nlines - 1 && l < npointers - 1; ++j, ++l)
		{
			...
			if (l % (npointers / state->width) == 0 && l / (npointers / state->width) < MAX_MEM_PARALLELISM)
			{
...
void tlb_initialize(iter_t iterations, void *cookie)
{
	...
	if (addr)
		bzero(addr, npages * sizeof(char **));
	if (pages)
		bzero(pages, npages * sizeof(char **));
